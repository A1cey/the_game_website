...
var require_dist = __commonJS({
	"node_modules/cookie/dist/index.js"(exports) {
		...
		function serialize2(name, val, options) {
			const enc =
				(options == null ? void 0 : options.encode) ||
				encodeURIComponent;
	...
	while (stack.length > 0) {
		...
		const partsForObj = obj =>
			Object.keys(obj)
				.map(
					k =>
						`"_${flatten.call(this, k)}":${flatten.call(this, obj[k])}`,
				)
				.join(",");
		let error = null;
		switch (typeof input2) {
			...
			case "symbol": {
				...
				if (!keyFor) {
					...
					str[index2] =
						`["${TYPE_SYMBOL}",${JSON.stringify(keyFor)}]`;
				}
				break;
			}
			case "object": {
				...
				if (!pluginHandled) {
					...
					if (isArray) {
						...
						str[index2] =
							`["${TYPE_URL}",${JSON.stringify(input2.href)}]`;
						...
						str[index2] =
							`["${TYPE_ERROR}",${JSON.stringify(input2.message)}`;
						...
						str[index2] =
							`["${TYPE_NULL_OBJECT}",{${partsForObj(input2)}}]`;
	...
	while (!read.done) {
		...
		switch (line[0]) {
			case TYPE_PROMISE: {
				...
				if (!deferred) {
					throw new Error(
						`Deferred ID ${deferredId} not found in stream`,
					);
				}
...
function raceSignal(promise, signal) {
	...
	if (signal.aborted)
		return Promise.reject(
			signal.reason || new Error("Signal was aborted."),
		);
...
function createRouter(init) {
	...
	function completeNavigation(
		...
		let preventScrollReset =
			...
			(state.navigation.formMethod != null &&
				isMutationMethod(state.navigation.formMethod) &&
				((_b = location.state) == null ? void 0 : _b._isRedirect) !==
					true);
...
function getMatchesToLoad(
	...
	let navigationMatches = boundaryMatches.filter((match, index) => {
		...
		if (initialHydration) {
			return shouldLoadRouteOnHydration(
				route,
				state.loaderData,
				state.errors,
			);
...
function patchRoutesImpl(
	...
	let newRoutes = convertRoutesToDataRoutes(
		...
		[
			...
			"patch",
			String(
				(childrenToPatch == null ? void 0 : childrenToPatch.length) ||
					"0",
			),
		],
		manifest,
	...
	let setState = React3.useCallback(
		(
			...
			let isViewTransitionAvailable =
				...
				typeof router2.window.document.startViewTransition ===
					"function";
			warnOnce(
	...
	return (routesByParentId[parentId] || []).map(route => {
		...
		if (routeModule) {
			...
			let initialError = hasInitialError
				? (_b = initialState == null ? void 0 : initialState.errors) ==
					null
					? void 0
					: _b[route.id]
				: void 0;
			let isHydrationRequest =
				...
				(((_c = routeModule.clientLoader) == null
					? void 0
					: _c.hydrate) === true ||
					!route.hasLoader);
...
function useFogOFWarDiscovery(router2, manifest, routeModules, isSpaMode) {
	React8.useEffect(() => {
		...
		if (
			...
			((_a = navigator.connection) == null ? void 0 : _a.saveData) ===
				true
		) {
			return;
		}
...
async function fetchAndApplyManifestPatches(
	...
	let manifestPath =
		`${basename != null ? basename : "/"}/__manifest`.replace(/\/+/g, "/");
...
import(${JSON.stringify(manifest.entry.module)});`;
	...
	let preloads = isHydrated
		? []
		: manifest.entry.imports.concat(routePreloads);
	return isHydrated
		? null
...
var Link = React10.forwardRef(function LinkWithRef(
	...
	return shouldPrefetch && !isAbsolute
		? React10.createElement(
				...
				null,
				link,
				React10.createElement(PrefetchPageLinks, { page: href }),
			)
		: link;
});
...
function useSearchParams(defaultInit) {
	...
	let defaultSearchParamsRef = React10.useRef(
...
function encodeLocation(to) {
	let href = typeof to === "string" ? to : createPath(to);
	href = href.replace(/ $/, "%20");
	let encoded = ABSOLUTE_URL_REGEX3.test(href)
		? new URL(href)
...
function createRoutesStub(routes, context = {}) {
	return function RoutesTestStub({
		...
		return React13.createElement(
			...
			React13.createElement(RouterProvider, {
			...
			routeModule.headers
				? typeof routeModule.headers === "function"
					? routeModule.headers({
							...
							parentHeaders,
							actionHeaders,
							errorHeaders: includeErrorHeaders
								? errorHeaders
								: void 0,
						})
					: routeModule.headers
